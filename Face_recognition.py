...
def draw_boundary(img, classifier, scaleFactor, minNeighbors, color, text, clf):
    ...
    for (x, y, w, h) in features:
        ...
        if id==1:
            ...
            cv2.putText(img, "Preyrit", (x, y-4), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 1, cv2.LINE_AA)
        elif id == 3:
            cv2.putText(img, "Simran", (x, y-4), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 1, cv2.LINE_AA)
        else:
